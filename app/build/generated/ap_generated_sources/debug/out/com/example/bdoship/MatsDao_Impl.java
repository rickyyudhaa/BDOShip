...
public final class MatsDao_Impl implements MatsDao {
  ...
  private final EntityInsertionAdapter __insertionAdapterOfMaterial_1;
  ...
  public MatsDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfMaterial_1 = new EntityInsertionAdapter<Material>(__db) {
      @Override
      public String createQuery() {
        return "INSERT OR REPLACE INTO `material_table`(`id`,`name`,`image`,`price`,`qtyperday`,`barter`,`daily`,`coin`) VALUES (nullif(?, 0),?,?,?,?,?,?,?)";
      }
  ...
  @Override
  public void insertAll(List<Material> mats) {
    __db.beginTransaction();
    try {
      __insertionAdapterOfMaterial_1.insert(mats);
      __db.setTransactionSuccessful();
    } finally {
      __db.endTransaction();
    }
  ...
  @Override
  public Material getbyid(int id_mats) {
    final String _sql = "SELECT * FROM material_table WHERE id = ?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    _statement.bindLong(_argIndex, id_mats);
    final Cursor _cursor = __db.query(_statement);
    try {
      final int _cursorIndexOfId = _cursor.getColumnIndexOrThrow("id");
      final int _cursorIndexOfName = _cursor.getColumnIndexOrThrow("name");
      ...
      if(_cursor.moveToFirst()) {
        ...
        _tmp_2 = _cursor.getInt(_cursorIndexOfCoin);
        _tmpCoin = _tmp_2 != 0;
        _result = new Material(_tmpName,_tmpImage,_tmpPrice,_tmpBarter,_tmpDaily,_tmpCoin,_tmpQtyperday);
        final int _tmpId;
        _tmpId = _cursor.getInt(_cursorIndexOfId);
